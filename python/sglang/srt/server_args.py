...
class ServerArgs:
    ...
    paged_experts_num_resident: A[
        str,
        "Resident experts per layer (K) for --enable-paged-experts: an int to pin K, or 'auto' to size "
        "from free VRAM (safe margins, respects --mem-fraction-static).",
    ] = "auto"
    ...
    paged_experts_eviction: A[
        ...
        Arg(
            ...
            "per step: LRU for single-token decode, LFU for batched steps (the measured winner in each "
            "regime). num_tokens is static per captured graph, so each bs bucket bakes its policy at "
            "capture.",
