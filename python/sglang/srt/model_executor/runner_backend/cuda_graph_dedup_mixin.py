    ...
    def replay(self, graph: DedupedCudaGraph, stream: int) -> None:
        ...
        if group.current_raw_graph != raw_graph:
            if group.launch_done_event is not None:
                checkCudaErrors(
                    cuda_rt.cudaEventSynchronize(group.launch_done_event)
                )
        ...
        if len(group.graphs) > 1:
            if group.launch_done_event is None:
                group.launch_done_event = checkCudaErrors(
                    cuda_rt.cudaEventCreateWithFlags(
                        cuda_rt.cudaEventDisableTiming
                    )
                )
            checkCudaErrors(
                cuda_rt.cudaEventRecord(group.launch_done_event, stream)
            )
