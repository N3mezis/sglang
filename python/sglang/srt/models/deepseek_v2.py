    ...
    def forward(
        ...
        if gateup_pre_quant is not None:
            ...
            if (
                ...
                and x.shape[0] <= 256
                # hybrid_int4_nvfp4 has no dense .weight; guard presence before .dtype (short-circuits).
...
class DeepseekV2AttentionMLA(
    ...
    def __init__(
        ...
        # Lazily computed on first forward (upstream). Its `not is_packed_weight` guard short-circuits
        # for hybrid_int4_nvfp4 (packed, no dense .weight), so fused_a_gemm_weight_eligible is never
        # reached on our int4 path — the presence guard we used eagerly is subsumed.
