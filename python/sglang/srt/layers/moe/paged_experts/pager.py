    ...
    if missed:
        ...
        for i, p in enumerate(missed):
            if p._refill_after_replay(
                counts[p._miss_idx], cold_logs[i], neededs[i], slot_exps[i]
            ):
                staged = True
    if staged:
...
class PagedExpertStore:
    ...
    def _setup_window_ondevice(self) -> None:
        ...
        set_window_profile_tokens(
            int(
                getattr(get_global_server_args(), "paged_experts_window_profile", 0)
                or 0
            )
        )
    ...
    def refresh_window_freq(self) -> None:
        ...
        self._log2cold_d.copy_(
            self.store.cold_pos.to(dtype=torch.int32, device=self.device)
        )
        self._freq_d.zero_()
