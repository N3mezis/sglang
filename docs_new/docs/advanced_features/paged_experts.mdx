...
page-locked when the whole store fits the pin budget — half of available host memory; on WSL2, which caps
total page-locked memory below host RAM at a box-dependent bound, the budget is verified against the
*measured* pin ceiling by probing real pinned allocations at startup; otherwise the largest window that
