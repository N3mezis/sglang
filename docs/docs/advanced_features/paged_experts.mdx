...
To push `K` higher than `auto` chooses, lower what it reserves rather than overriding `K` blindly:
`--paged-experts-kv-reserve-gb` is the K/KV split knob, and on a hybrid (mamba / linear-attention) model
it is also the total budget for the per-request state cache. `auto` sizes both terms from the *declared*
concurrency (`--max-running-requests` × context), so a single-stream config already reserves little; if
you have pinned a smaller `--max-mamba-cache-size` than `auto` assumes, it is honoured verbatim.
