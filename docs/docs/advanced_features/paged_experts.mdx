...
At low concurrency the dominant decode cost is **host-side latency**, not PCIe bandwidth: assembling each
step's non-resident ("cold") experts on the host and issuing the copies. On a typical host-RAM-backed store
the PCIe link runs at a small fraction of its capacity and the GPU is partly idle each step, so decode is
**host/latency-bound rather than bandwidth-bound** — which shapes its performance profile (see
...
Because the dominant decode cost is the fixed per-step host work (gathering and staging cold experts), not
PCIe bandwidth, single-stream latency is modest but **aggregate throughput scales with concurrency**:
batching amortizes that fixed per-step cost over more tokens, and a single cold-expert read serves the whole
batch. The ceiling is where the host gather / GPU compute saturates, not the PCIe link. Choose the operating
point for your workload — low concurrency for per-user latency, high concurrency for aggregate throughput.

Tuning guidance:
- For **throughput / many users**, raise `--max-running-requests`; aggregate throughput scales with
  concurrency until the host gather / GPU compute saturates.
