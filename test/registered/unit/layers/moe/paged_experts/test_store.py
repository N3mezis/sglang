...
class TestWindowedExpertStore(CustomTestCase):
    ...
    def test_mixed_page_in(self):
        ...
        for name in store.gpu:
            self.assertTrue((store.gpu[name].data[0] == 2.0).all().item())  # expert 1 (hot)
            self.assertTrue((store.gpu[name].data[3] == 7.0).all().item())  # expert 6 (cold)
        ...
        torch.cuda.synchronize()
        for name in store.gpu:
            self.assertTrue((store.gpu[name].data[1] == 3.0).all().item())  # expert 2 (hot)
            self.assertTrue((store.gpu[name].data[2] == 8.0).all().item())  # expert 7 (cold)
    ...
    def test_disk_cold_backing(self):
        ...
        for name in store.gpu:
            self.assertTrue(store.host_hot[name].is_pinned())  # hot window still page-locked
            self.assertFalse(store.host_cold[name].is_pinned())  # cold tail is the disk mmap
            full = torch.randn((E, *store.gpu[name].shape[1:]))
            store.fill_tensor(name, full)
            for e in range(E):
                self.assertTrue(torch.equal(store.row(name, e).cpu(), full[e]))
        ...
        for name in store.gpu:
            ...
            self.assertTrue((store.gpu[name].data[3] == 7.0).all().item())  # cold (from disk mmap)
